...
- Add opt-in daily private R2 aggregate backups with immutable conditional writes, bounded byte-preserving archives, independent read-only restore, original-day three-calendar-month expiry, native lifecycle planning, and a disabled-by-default main-only workflow. Require operator lifecycle/readback and roundtrip proof before enabling; no raw-event replay or full-dimensional backup.
