...
already exist; explicitly resolve symlinked parents to their real paths.
Output must remain outside the executing telemetry source checkout and every linked worktree,
including nested repositories and differently cased paths on case-insensitive filesystems.
This boundary is checked before inspecting an existing bundle, reading credentials,
creating directories, or making requests. Private directories and unrelated private
repositories outside those checkouts remain valid destinations. Execution needs
...
historical exporter in a private owned temporary directory outside the bundle and
all telemetry source checkouts. The scratch parent is checked before any temporary
directory is created, using physical directory identity so case aliases cannot
